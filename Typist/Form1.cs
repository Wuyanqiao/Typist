...
public partial class Form1 : Form
{
    ...
    protected override void WndProc(ref Message message)
    {
        if (message.Msg == WmHotKey)
        {
            ...
            if (hotKeyId == StartHotKeyId) { _ = StartTypingAsync(); return; }
            if (hotKeyId == StopHotKeyId) { StopTyping(); return; }
        }
    ...
    private void BuildInterface()
    {
        ...
        selectTargetButton = new Button { Text = "选择目标", Width = 110, Height = 34 };
        selectTargetButton.Click += async (_, _) => await SelectTargetAsync();
        clearTargetButton = new Button { Text = "清除目标", Width = 110, Height = 34 };
        ...
        delayPerCharacterInput = new NumericUpDown { Minimum = 0, Maximum = 5000, Value = 60, Increment = 10, Dock = DockStyle.Fill };
        startDelayInput = new NumericUpDown { Minimum = 0, Maximum = 60, Value = 5, Dock = DockStyle.Fill };
        enterForLineBreaksCheckBox = new CheckBox { Text = "换行使用 Enter", AutoSize = true, Checked = true, Margin = new Padding(0, 8, 20, 4) };
        minimizeBeforeTypingCheckBox = new CheckBox { Text = "开始后最小化窗口", AutoSize = true, Checked = true, Margin = new Padding(0, 8, 20, 4) };

        startButton = new Button { Text = "开始", Width = 96, Height = 34 };
        startButton.Click += async (_, _) => await StartTypingAsync();
        stopButton = new Button { Text = "停止", Width = 96, Height = 34 };
        stopButton.Click += (_, _) => StopTyping();
        clearButton = new Button { Text = "清空", Width = 96, Height = 34 };
        ...
        typingProgressBar = new ProgressBar { Dock = DockStyle.Fill, Height = 18, Style = ProgressBarStyle.Continuous };
        statusLabel = new Label { Text = "就绪", AutoSize = true, Margin = new Padding(0, 8, 0, 0) };
        hotKeyStatusLabel = new Label { AutoSize = true, ForeColor = Color.FromArgb(90, 96, 108), Margin = new Padding(0, 4, 0, 0) };
        ...
        var footer = new TableLayoutPanel { Dock = DockStyle.Fill, AutoSize = true, ColumnCount = 1, RowCount = 3 };
    ...
    {
        panel.Controls.Add(new Label { Text = labelText, AutoSize = true, Anchor = AnchorStyles.Left, Margin = new Padding(0, 0, 8, 0) }, column, 0);
        panel.Controls.Add(control, column + 1, 0);
    }

    private async Task StartTypingAsync()
    {
        if (typingCancellation is not null) return;
        var text = draftTextBox.Text;
        if (string.IsNullOrEmpty(text)) { statusLabel.Text = "草稿区为空"; return; }
        ...
        try
        {
            ...
            if (minimizeBeforeTypingCheckBox.Checked) WindowState = FormWindowState.Minimized;
            ...
            {
                typingProgressBar.Maximum = Math.Max(p.TotalCharacters, 1);
                typingProgressBar.Value = Math.Min(p.SentCharacters, typingProgressBar.Maximum);
                statusLabel.Text = $"输入中 {p.SentCharacters}/{p.TotalCharacters}";
            });

            await typingEngine.TypeAsync(text, options, progress, token);
            statusLabel.Text = "完成";
        }
        catch (OperationCanceledException) { statusLabel.Text = "已停止"; }
        catch (Exception ex) { statusLabel.Text = ex.Message; WindowState = FormWindowState.Normal; Activate(); }
        finally
        {
    ...
    private void StopTyping() => typingCancellation?.Cancel();
    ...
    private void ClearTarget() { selectedTarget = null; UpdateTargetStatus(); statusLabel.Text = "已清除目标位置"; }
    ...
    private void UpdateTypingState(bool isTyping)
    {
        ...
        if (!isTyping) typingProgressBar.Value = 0;
    }
